...
# Funções auxiliares (conformes discutidas acima, completas e integradas)
...
if btn_atualizar:
    st.cache_data.clear()
    # Uso seguro do experimental_rerun com fallback
    try:
        st.experimental_rerun()
    except AttributeError:
        from streamlit import runtime
        raise runtime.scriptrunner.script_runner.RerunException(st.runtime.get_script_run_ctx())

superficie_en = superficies_map[superficie_pt]
url_torneio_selec = next(t["url"] for t in torneios if t["nome"] == torneio_selec)

# Carregar bases Elo e yElo
...
with tab_manual:
    ...
    with colA:
        ...
        if (
            ODD_MAX >= odd_a >= ODD_MIN
            and (VALOR_MIN - TOLERANCIA) <= valor_a_arred <= (VALOR_MAX + TOLERANCIA)
        ):
            classe_stake = (
                "stake-low"
                if stake_a == 5
            ...
            st.markdown(
                f"<span class='faixa-stake {classe_stake}'>Stake recomendada: €{stake_a:.2f}</span>",
    ...
    with colB:
        ...
        if (
            ODD_MAX >= odd_b >= ODD_MIN
            and (VALOR_MIN - TOLERANCIA) <= valor_b_arred <= (VALOR_MAX + TOLERANCIA)
        ):
            classe_stake = (
                "stake-low"
                if stake_b == 5
            ...
            st.markdown(
                f"<span class='faixa-stake {classe_stake}'>Stake recomendada: €{stake_b:.2f}</span>",
                unsafe_allow_html=True,
            )
            st.success("Valor positivo ✅")
        else:
...
- Stake recomendada segundo valor esperado:
