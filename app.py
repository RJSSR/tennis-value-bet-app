...
with tab_manual:
    ...
    with colA:
        ...
        if ODD_MAX >= odd_a >= ODD_MIN and (
            VALOR_MIN - TOLERANCIA
        ) <= valor_a_arred <= (VALOR_MAX + TOLERANCIA):
            classe_stake = (
                "stake-low"
                if stake_a == 5
                else "stake-mid"
    ...
    with colB:
        ...
        if ODD_MAX >= odd_b >= ODD_MIN and (
            VALOR_MIN - TOLERANCIA
        ) <= valor_b_arred <= (VALOR_MAX + TOLERANCIA):
            classe_stake = (
                "stake-low"
                if stake_b == 5
                else "stake-mid"
...
with tab_hist:
    ...
    if not df_hist.empty:
        ...
        selecionado_idx = st.selectbox(
            "Escolher aposta para atualizar", options=range(len(opcoes_evento)), format_func=lambda i: opcoes_evento[i]
        )
