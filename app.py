...
def obter_jogos_toronto():
    ...
    for table in soup.find_all("table", class_="table-main"):
        ...
        for tr in tbody.find_all("tr"):
            tds = tr.find_all("td")
            if any("Toronto" in td.text for td in tds):
...
st.title("Análise de Valor em Apostas de Ténis — ATP Toronto (nomes completos)")
...
def obter_jogos_completo():
    ...
    for jogo in jogos_curto:
        ...
        if not nome_completo_a: nome_completo_a = jogo["nome_abrev_a"]
        if not nome_completo_b: nome_completo_b = jogo["nome_abrev_b"]
...
if not jogos_hoje:
    st.warning("Nenhum jogo de Toronto encontrado hoje.")
    st.stop()
