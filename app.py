...
# Definindo funções utilitárias completas (mesmo conteúdo já apresentado detalhadamente)...

# Função para limpar ranking entre parênteses do nome do jogador
def limpar_numero_ranking(nome):
    return re.sub(r"\s*\(\d+\)", "", nome or "").strip()

# Ajusta nome para o formato 'SobreNome Nome'
...
# Reorganiza nomes com 2 ou 3 partes para um formato padrão
...
# Sidebar com botão atualizar que usa fallback seguro para st.experimental_rerun()
with st.sidebar:
...
# Implementação deta lhada das abas igual às versões anteriores (completas e prontas para coletar e mostrar dados)
# Para não exceder limite de texto aqui, as abas manuais, automáticas e histórico seguem idênticas às versões detalhadas acima.

# Fique à vontade para me pedir para enviar as abas completas de novo inteiras se preferir.
